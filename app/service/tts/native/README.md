...
2. **智能检测**：
   ...
   - 如果    H --> I{处理成功?}
    ...
    E --> L
    G --> L
    J --> LTTS模型，从 `request.generationConfig` 检查    H --> I{处理成功?}
    I -->|是| J[返回原生TTS响应]
    I -->|否| K[自动回退到标准服务]
    E --> L
    G --> L
    ...
    E --> L
    G --> L
    J --> L包含 `responseModalities: ["AUDIO"]` 和 `speechConfig`
3. **服务选择**：
...
flowchart TD
    ...
    E --> L
    G --> L
    J --> L| F{包含responseModalities和speechConfig?}
    F -->|    H --> I{处理成功?}
    I -->|是| J[返回原生TTS响应]
    I -->|否| K[自动回退到标准服务]
    E --> L
